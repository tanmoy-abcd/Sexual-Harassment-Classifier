...
def predict():
    #loading the pretrained model and vectoriser
    model = joblib.load('model.pkl')
    vectoriser = joblib.load('vectoriser.pkl')
    
    #getting the raw text from the request
    to_predict_list = flask.request.form.to_dict()   
    description = to_predict_list['description']
    
    preprocessed_text = preprocess(description)
    ...
    for i in pred: 
        ...
        if len(result) == 0 :
            result.append('Sorry , I Cannot Classify this into Commenting , Staring or Groping')

    return flask.jsonify({'predictions' : ' , '.join(result)})
